...
class HyperbolicGraphConvolution(nn.Module):
    ...
    def forward(self, input, old):
        x, adj = input
        h = self.linear.forward(x, old)
        h = self.agg.forward(h, adj, old)
        h = self.hyp_act.forward(h, old)
        output = h, adj
        return output
...
class HypLinear(nn.Module):
    ...
    def forward(self, x, old):
        ...
        if self.use_bias:
            # Normed bias
            #normed_bias = self.bias.view(1, -1)/(self.bias.view(1, -1).norm(p=2)+1e-08)
            #hyp_bias = self.manifold.expmap0(normed_bias, self.c)
...
class HypAct(Module):
    ...
    def forward(self, x, old):
        if old:
            if self.act == F.leaky_relu:
                xt = self.act(self.manifold.logmap0(x, c=self.c_in), negative_slope=0.2)
            else:
            ...
            if self.act == F.leaky_relu:
                xt = self.act(x, negative_slope=0.2)
            else:
                xt = self.act(x)
            return xt
