...
This repository is a graph representation learning library, containing an implementation of Hyperbolic Graph Convolutions [[1]](http://web.stanford.edu/~chami/files/hgcn.pdf) in PyTorch, as well as GCN method.
...
We provide examples of training commands used to train HGCN and GCN graph embedding models for link prediction and node classification. In the examples below, we used a fixed random seed set to 1234 for reproducibility purposes. Note that results might slightly vary based on the machine used. To reproduce results in the paper, run each command for 10 random seeds, specifically from 0 to 9, and average the results.
